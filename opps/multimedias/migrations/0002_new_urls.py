...
from opps.multimedias.models import app_namespace
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Load audios pk
        audios_pk = orm.Audio.objects.values_list('container_ptr', flat=True)

        # Load videos pk
        videos_pk = orm.Video.objects.values_list('container_ptr', flat=True)

        # Audios
        audios = orm['containers.Container'].objects.filter(
            pk__in=audios_pk
        )
        for audio in audios:
            new_path = reverse(
                u'{0}:audio_detail'.format(app_namespace),
                kwargs={
                    'channel__long_slug': audio.channel_long_slug,
                    'slug': audio.slug
                }
            )
        ...
        for video in videos:
            new_path = reverse(
                u'{0}:video_detail'.format(app_namespace),
                kwargs={
                    'channel__long_slug': video.channel_long_slug,
                    'slug': video.slug
                }
            )
    ...
    complete_apps = ['redirects', 'multimedias']
    symmetrical = True
